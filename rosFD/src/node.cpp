...
bool callbackFuncSrv(
    package::ServiceName::Request &req.package::ServiceName::Response &res) {
...
void callbackFuncAct(const package::NameGoalConstPtr &goal_,
...
int main(int argc, char **argv) {
  ...
  // Publisher node
  ros::Publisher publisher_object_ =
      nh_.advertise<msg_type>("topic_name", 1000);
  ...
  // Subscriber node
  ros::Subscriber subscriber_object_ =
      nh_.subscribe("topic_name", 1000, callbackFuncSub);
  ros::spin();
  // ----------------------------

  // Service node
  ros::ServiceServer service_object_ =
      nh_.advertiseService("service_name", callbackFuncSrv);
  ros::spin();
  // ----------------------------

  // Client node - calling service
  ros::ServiceClient client_object_ =
      nh_.serviceClient<package::ServiceName>("service_name");
  ...
  // Action Server node
  actionlib::SimpleActionServer<package::NameAction> server_object_(
      nh_, boost::bind(&callbackFuncAct, _1, &server_object_), false);
  ...
  // Action Client node
  actionlib::SimpleActionClient<package::NameAction> client_object_(
      "action_name", true); // true => don't need ros::spin();
